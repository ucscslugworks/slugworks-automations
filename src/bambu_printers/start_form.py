import logging
import os
import time

from google.auth.transport.requests import Request
from google.oauth2.credentials import Credentials
from google_auth_oauthlib.flow import InstalledAppFlow
from googleapiclient.discovery import build
from googleapiclient.errors import HttpError

SCOPES = ["https://www.googleapis.com/auth/spreadsheets"]
START_FORM_SHEET_ID = ""
EXPECTED_ROW_LENGTH = 3


class StartForm:
    def __init__(self, logger: logging.Logger):
        self.logger = logger
        path = os.path.dirname(os.path.abspath(__file__))

        creds = None
        # The file token.json stores the user's access and refresh tokens, and is
        # created automatically when the authorization flow completes for the first
        # time.
        if os.path.exists(path + "/../../common/token.json"):
            creds = Credentials.from_authorized_user_file(
                path + "/../../common/token.json", SCOPES
            )
        elif not os.path.exists(path + "/../../common/credentials.json"):
            self.logger.error("StartForm: No credentials.json file found.")
            exit(1)
        # If there are no (valid) credentials available, let the user log in (assuming credentials.json exists).
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                creds.refresh(Request())
            else:
                flow = InstalledAppFlow.from_client_secrets_file(
                    path + "/../../common/credentials.json", SCOPES
                )
                creds = flow.run_local_server(port=44649)
            # Save the credentials for the next run
            with open(path + "/../../common/token.json", "w") as token:
                token.write(creds.to_json())

        try:
            service = build("sheets", "v4", credentials=creds)

            # Call the Sheets API
            self.g_sheets = service.spreadsheets()

        except HttpError as e:
            self.logger.error(f"StartForm init: {e}")
            exit(1)

        self.latest_row = 0

    def get(self):
        try:
            result = (
                self.g_sheets.values()
                .get(spreadsheetID=START_FORM_SHEET_ID, range=f"A{self.latest_row}:C")
                .execute()
            )

            values = result.get("values", [])

            if not values:
                self.logger.info("StartForm: No new data found.")
                return None

            self.latest_row += len(values)

            for i in range(len(values)):
                values[i].extend([""] * (EXPECTED_ROW_LENGTH - len(values[i])))

                values[i][0] = time.mktime(
                    time.strptime(str(values[i][0]), "%m/%d/%Y %H:%M:%S")
                )

                if len(values[i][1]) > 0:
                    values[i][1] = str(values[i][1]).lower().split("@ucsc.edu")[0]

                values[i][2] = float(values[i][2])

                print(values[i])

            self.logger.info(f"StartForm: Got {len(values)} new rows.")
            return values
        except HttpError as e:
            self.logger.error(f"StartForm.get: {e}")
            return None
